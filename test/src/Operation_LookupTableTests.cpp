...
namespace UnitTests
{
	...
	{
		ASSERT_FLOAT_EQ(-10, (_operation->Execute<float, float>(0.0f)));

		ASSERT_FLOAT_EQ(20, (_operation->Execute<float, float>(0.99f)));

		ASSERT_NEAR(0, (_operation->Execute<float, float>(0.33f)), 0.001f);

		ASSERT_NEAR(-1.25f, (_operation->Execute<float, float>(0.28875f)), 0.001f);
	}

	TEST_F(Operation_LookupTableTests, WhenGettingValueAboveMaxValue_ThenCorrectValueIsReturned)
	{
		ASSERT_FLOAT_EQ(90, (_operation->Execute<float, float>(100.0f)));
	}

	TEST_F(Operation_LookupTableTests, WhenGettingValueBelowMinValue_ThenCorrectValueIsReturned)
	{
		ASSERT_FLOAT_EQ(-10, (_operation->Execute<float, float>(-1.0f)));
	}
}
