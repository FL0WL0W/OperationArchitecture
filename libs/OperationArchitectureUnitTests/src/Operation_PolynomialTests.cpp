...
namespace UnitTests
{
	...
	TEST_F(Operation_PolynomialTest, WhenGettingValueWithinLimits_ThenCorrectValueIsReturned)
	{
		...
		ASSERT_FLOAT_EQ(20, _operation->Execute(Variable(1.0f)).To<float>());

		ASSERT_FLOAT_EQ(-1.25f, _operation->Execute(Variable(0.5f)).To<float>());
	}

	TEST_F(Operation_PolynomialTest, WhenGettingValueAboveMaxValue_ThenCorrectValueIsReturned)
	{
		ASSERT_FLOAT_EQ(150, _operation->Execute(Variable(100.0f)).To<float>());
	}

	TEST_F(Operation_PolynomialTest, WhenGettingValueBelowMinValue_ThenCorrectValueIsReturned)
	{
		ASSERT_FLOAT_EQ(-40, _operation->Execute(Variable(-100.0f)).To<float>());
	}
}
